...
set WIN_ZIP=SPT%SPT_VER%_%date:~-2,2%%date:~-10,2%%date:~-7,2%_win.zip
...
msbuild %BUILD_PATH%\%ADL_VER%\as-spt%SPT_VER%.vcxproj /p:Configuration=Release || goto :error
msbuild %BUILD_PATH%\%ADL_VER%\objdump-spt%SPT_VER%.vcxproj /p:Configuration=Release || goto :error

MD %SPT_BIN%\bin
MD %SPT_BIN%\doc
MD %SPT_BIN%\inc

copy /B %RELEASE_PATH%\as-spt%SPT_VER%.exe %SPT_BIN%\bin\as-spt.exe
copy /B %RELEASE_PATH%\objdump-spt%SPT_VER%.exe %SPT_BIN%\bin\objdump-spt.exe
copy /Y %SPT_SRC%\util\win\port-asm3.1.exe %SPT_BIN%\bin
