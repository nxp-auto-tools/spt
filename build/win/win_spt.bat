...
IF NOT DEFINED ADL_VER        SET ADL_VER=adl-3.17.3
...
patch.exe -p1 -i ..\..\%SPT_SRC%\build\patch_adl_3.17.3.txt --binary -t
CD ..\..
...
COPY /Y %SPT_SRC%\build\win\vs2013\* %BUILD_PATH%\%ADL_VER%\
...
msbuild %BUILD_PATH%\%ADL_VER%\as-spt3.0.vcxproj /p:Configuration=Release      || goto :error
msbuild %BUILD_PATH%\%ADL_VER%\objdump-spt3.0.vcxproj /p:Configuration=Release      || goto :error

MD %SPT_BIN%\bin
MD %SPT_BIN%\doc
MD %SPT_BIN%\inc

copy /B %RELEASE_PATH%\as-spt3.0.exe %SPT_BIN%\bin\as-spt.exe
copy /B %RELEASE_PATH%\objdump-spt3.0.exe %SPT_BIN%\bin\objdump-spt.exe
