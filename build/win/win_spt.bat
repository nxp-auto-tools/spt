...
IF NOT DEFINED ADL_VER        SET ADL_VER=adl-3.24.4_patch
...
set WIN_ZIP=SPT3.5_%date:~-2,2%%date:~-10,2%%date:~-7,2%_win.zip
...
msbuild %BUILD_PATH%\%ADL_VER%\as-spt3.5.vcxproj /p:Configuration=Release || goto :error
msbuild %BUILD_PATH%\%ADL_VER%\objdump-spt3.5.vcxproj /p:Configuration=Release || goto :error

MD %SPT_BIN%\bin
MD %SPT_BIN%\doc
MD %SPT_BIN%\inc

copy /B %RELEASE_PATH%\as-spt3.5.exe %SPT_BIN%\bin\as-spt.exe
copy /Y %SPT_SRC%\util\win\port-asm2.8.exe %SPT_BIN%\bin
copy /B %RELEASE_PATH%\objdump-spt3.5.exe %SPT_BIN%\bin\objdump-spt.exe
copy /Y %SPT_SRC%\inc\*.inc %SPT_BIN%\inc
copy /Y %SPT_SRC%\doc\as-spt3.5-manual.pdf %SPT_BIN%\doc
copy /Y %SPT_SRC%\doc\spt3.5-mnemonics-quickref.txt %SPT_BIN%\doc
