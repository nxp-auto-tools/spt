...
[ "${ADL_ROOT}" = "" ]           && ADL_ROOT="/opt/adl-3.24.4_patch"
...
#copy binaries
cp dev/as-spt3.4 $SPT_BIN/bin/as-spt
cp dev/objdump-spt3.4 $SPT_BIN/bin/objdump-spt
#copy SPT2.8 asm porting utility
cp util/linux/port-asm2.8 $SPT_BIN/bin
#copy includes directory
cp -r inc/*.inc $SPT_BIN/inc
#copy documentation
cp doc/as-spt3.4-manual.pdf $SPT_BIN/doc
cp doc/spt3.4-mnemonics-quickref.txt $SPT_BIN/doc
cp doc/gas-manual.pdf $SPT_BIN/doc
cp doc/readme.pdf $SPT_BIN/readme.pdf
#prepare artifact
BUILD_DATE=`date +%y%m%d`
zip -q -r SPT3.4_${BUILD_DATE}_linux.zip $SPT_BIN
