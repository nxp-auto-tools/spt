...
bool acb_apply_fix(fixS *fixP, valueT *valP, segT seg ATTRIBUTE_UNUSED)
{
  ...
  if ((rtype == R_AARCH64_PREL32 || rtype == R_ARM_REL32_NOI)
      && !strcmp(seg->name, ".text")) {
    if (is_32bit_ARM)
      fixP->fx_r_type = (bfd_reloc_code_real_type)R_ARM_REL32_NOI;
    else
      fixP->fx_r_type = (bfd_reloc_code_real_type)R_AARCH64_PREL32;
  }
...
This program has absolutely no warranty.\n"));
  ...
  if (arg != 0L && !strcmp(arg, "verbose") && !printed) {
    fprintf(stderr, _("GNU assembler %s version %s for SPT3.1\n"),
            BFD_VERSION_STRING, adl_asm_version.c_str());
    printed = 1;
    return 1;
  }
